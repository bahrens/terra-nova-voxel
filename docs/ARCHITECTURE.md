...
styles.css        HUD, menu, crosshair, hotbar, underwater overlay
js/
  main.js         renderer, HUD/hotbar, pointer-lock, save/load, main loop, fog
  world.js        chunk streaming, terrain/biome/cave gen, water sim, edit diffs
  chunk.js        voxel storage + mesher (face cull, AO, water heights)
  player.js       FPS controls, AABB collision, DDA block raycast
...
### "Lighting" today
- The world uses **unlit** `MeshBasicMaterial`. `sky.js` fakes lighting by scaling
  each material's color by a time-of-day brightness scalar. There is **no
  per-block light propagation yet** — that's the lighting roadmap item, and the
  prerequisite for torches.
